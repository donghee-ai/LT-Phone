...
class SyncInterface(context: Context) {
    ...
    private fun resolveSyncDirectory(context: Context): File {
        ...
        if (!fallbackDir.exists() && !fallbackDir.mkdirs()) {
            Log.e(TAG, "Failed to create sync directory")
        }
        return fallbackDir
    }
    ...
    private fun performSyncAsync(
        ...
        executor.execute {
            ...
            try {
                ...
                Log.i(TAG, "$operation completed: success=$success")
    ...
    private fun mirrorSyncFilesToSafDir() {
        try {
            ...
            Log.w(TAG, "SAF mirror failed (non-fatal)")
        }
    }
    ...
    private fun copySyncFilesToSafDir() {
        ...
        if (safDir == null || !safDir.isDirectory) {
            Log.w(TAG, "SAF directory not accessible or not a directory")
            return
        }

        val counts = intArrayOf(0, 0) // [copied, skipped]
        mirrorDirectory(File(syncDir), safDir, counts)
        Log.i(TAG, "SAF mirror completed")
    }
    ...
    private fun mirrorDirectory(sourceDir: File, destDir: DocumentFile, counts: IntArray) {
        ...
        for (entry in entries) {
            if (cancelRequested) {
                Log.i(TAG, "SAF mirror cancelled")
                return
            }
            try {
                if (entry.isDirectory) {
                    ...
                    val subDir = when {
                        ...
                        existing != null -> {
                            Log.w(TAG, "SAF entry exists but is not a directory")
                            counts[1]++
                            continue
                        }
                    ...
                    if (subDir == null) {
                        Log.w(TAG, "Could not create SAF directory")
                        counts[1]++
                        continue
                    }
                    ...
                    if (existingFile != null && existingFile.isDirectory) {
                        Log.w(TAG, "SAF entry is a directory; cannot write a file there")
                        counts[1]++
                        continue
                    }
                    ...
                    if (dest == null) {
                        Log.w(TAG, "Could not create SAF file")
                        counts[1]++
                        continue
                    }
                    val out = appContext.contentResolver.openOutputStream(dest.uri, "wt")
                    if (out == null) {
                        Log.w(TAG, "Null output stream in SAF directory")
                        counts[1]++
                        continue
                    }
                ...
                Log.w(TAG, "Failed to copy entry to SAF directory")
                counts[1]++
            } catch (e: SecurityException) {
                Log.w(TAG, "Permission denied copying entry to SAF directory")
                counts[1]++
            }
        }
    }
