...
import net.activitywatch.android.watcher.InteractionWatcher
...
class EventParsingWorker(context: Context, params: WorkerParameters) : Worker(context, params) {
    ...
    override fun doWork(): Result {
        ...
        return try {
            ...
            watcher.processEventsSinceLastUpdate()
            // LT: 알람이 배터리 최적화에 막혀 놓친 afk 구간을 여기서 메운다.
            InteractionWatcher(applicationContext).processSince()
            Log.i(TAG, "Successfully processed events")
            Result.success()
        } catch (e: Exception) {
            Log.e(TAG, "Error processing events; retrying later")
            Result.retry()
        }
    }
}
