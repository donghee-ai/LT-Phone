...
HAS_SECRETS = $(shell test -n "$$ANDROID_KEYSTORE_PATH" && test -n "$$ANDROID_KEYSTORE_PASSWORD" && test -n "$$ANDROID_KEY_ALIAS" && test -n "$$ANDROID_KEY_PASSWORD" && echo 'true' || echo 'false')
...
test-e2e-adb:
	...
	adb shell am instrument -w \
		...
		net.lifetrainer.awphone.debug.test/androidx.test.runner.AndroidJUnitRunner
...
dist/aw-android.aab: $(AABDIR)/$(RELEASE_TYPE)/mobile-$(RELEASE_TYPE).aab
	mkdir -p dist
	@# Only sign if the public release keystore variables are set.
...
$(JNILIBS): $(JNI_arm7)/libaw_server.so $(JNI_arm8)/libaw_server.so $(JNI_x86)/libaw_server.so $(JNI_x64)/libaw_server.so \
	...
	python3 scripts/sanitize_native_paths.py $@ \
...
RUSTFLAGS_ANDROID="-C debuginfo=2 -Awarnings --remap-path-prefix=$(HOME)=/build --remap-path-prefix=$(CURDIR)=/src/lt-phone"
...
#  `--remap-path-prefix` keeps developer usernames and local workspace paths out of packaged native binaries.
...
$(WEBUI_DISTDIR):
	...
	if [ "$$SKIP_WEBUI" == "true" ]; then \
		...
		python3 scripts/sanitize_webui_dist.py $(WEBUI_DISTDIR); \
	fi
